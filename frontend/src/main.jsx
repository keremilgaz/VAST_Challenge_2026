...
function App() {
  ...
  const networkQuery = useMemo(() => {
    ...
    if (isNetworkFollowingHeatmapSort) {
      ...
      // OFF: network は filter 的には独立だが、crisis timeline は「全ビュー共通の時間軸」
      // なので時間窓だけは timeline に追従させる（slider を引くと network も同じ期間になる）。
      // timeline を切った（drive views OFF）ときだけ network 専用の手動 time range を使う。
  ...
  const selectedSemantic = selectedCellData
    ? (semanticComparisonMode === 'previous' ? selectedCellData.semantic_distance_prev : selectedCellData.semantic_distance_next)
    : null;
            ...
            {(
              <div className="fp-body">
                ...
                <Collapsible title="Heatmap mode" defaultOpen={true}>
                  ...
                  {heatmapMode === 'semantic_change' && (
                    <div className="sub-control">
                      <label className="check"><input type="radio" name="sem" checked={semanticComparisonMode === 'previous'} onChange={() => setSemanticComparisonMode('previous')} /> Compare with previous bucket</label>
                      <label className="check"><input type="radio" name="sem" checked={semanticComparisonMode === 'next'} onChange={() => setSemanticComparisonMode('next')} /> Compare with next bucket</label>
                    </div>
                  )}
                </Collapsible>
                    ...
                    <React.Fragment key={agent.agent_id}>
                      ...
                      {buckets.map(b => {
                        ...
                        if (heatmapMode === 'count') {
                          ...
                          const dist = c ? (semanticComparisonMode === 'previous' ? c.semantic_distance_prev : c.semantic_distance_next) : null;
            ...
            {(
              <div className="fp-body">
                ...
                <Collapsible title="Time range / sorting" defaultOpen={false}>
                  {(isNetworkFollowingHeatmapSort || timelineActive)
                    ? <div className="muted small">Time window comes from the crisis timeline above (drag the slider). Turn off “drive views” on the timeline to set a network-only range here.</div>
                    : <div className="muted small">Network-only time range (crisis timeline is off).</div>}
                  <div className={`time-inputs ${(isNetworkFollowingHeatmapSort || timelineActive) ? 'disabled' : ''}`}>
                  ...
                  <div className={`time-actions ${(isNetworkFollowingHeatmapSort || timelineActive) ? 'disabled' : ''}`}>
    ...
    <div className="detail-card">
      <div className="detail-summary" onClick={() => setCollapsed(c => !c)}>
        ...
        <div className="ds-text">
          ...
          <span className="ds-pipe">|</span> Semantic Δ ({semanticComparisonMode}): {selectedSemantic == null ? '—' : selectedSemantic.toFixed(2)}
        </div>
